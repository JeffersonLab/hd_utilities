...
int format_jobs_data(){
  ...
  while(IN >> id >> run >> file >> jobId
    ...
    // cout << "hostname = " << hostname << " status = " << status << " exitCode = " << exitCode << " result = " << result << endl;
    ...
    // cout << nTotal << " " << run << " " << file << " " << hostname << " " << status << "............" << endl;
    if(hostname == "NULL" && status == "PENDING"){
      ...
      // two categories will be missing from
      // walltime, cput, mem, vmem
      IN >> walltime >> cput;
      mem  = "-999";
      vmem = "-999";
    }else if(isStatusNULL == false){
      // cout << "in normal processing " << endl;
      ...
      // cout << "walltime = " << walltime << " cput = " << cput << " mem = " << mem << " vmem = " << vmem << endl;
      // Need to get "error". This can be either NULL or "Job timed out."
      // cout << "mem = " << mem << " vmem = " << vmem << endl;
      IN >> error;
      if(error == "NULL"){
	errorCode = 0;
      }
    ...
    } // end of hostname was not NULL

    // Get final 3 entries after error
    IN >> nevents >> timeCopy >> timePlugin;
    // cout << "nevents = " << nevents << " timeCopy = " << timeCopy << " timePlugin = " << timePlugin << endl;
    ...
    // cout << run << "   " << file << "   " << mem_int << "   " << vmem_int << endl;
