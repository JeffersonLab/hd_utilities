...
# BASE_DIR = "/home/fs7/jzarling/MCPhotonEffOut/Gun_plus_pipiP_match_off/"
# BASE_DIR = "/home/fs7/jzarling/MCPhotonEffOut/Gun_plus_pipiP_6deg_G4/"
BASE_DIR = "/home/fs7/jzarling/MCPhotonEffOut/Gun_plus_pipiP_scantheta_G4/"
...
def main(argv):
	...
	while current_val < MAX_VAL:
		...
		ascii_pruned = "pruned_gamma_pipip_"+str(current_val)+".ascii"
		pruned_hddm = "pruned_gamma_pipip_"+str(current_val)+".hddm"
		...
		SubmitQSubJob(ascii_pruned,pruned_hddm,current_val,controlin_loc,int(EVENTS_PER_val),RUN,gen_interactive)
		
		
		
		
		# shell_exec("rm " + ascii_to_prune , "")
		# shell_exec("rm pruned_gamma_p_"+str(e_gamma)+".ascii" , "")
...
def SubmitQSubJob(ascii_file,hddm_name,e_gamma,controlin_loc,NEVENTS,run,gen_interactive):
	...
	with open("ReplaceAndRun.csh", "w") as myfile:
		...
		if(HADRONIC_CLUSTER_VETO): myfile.write("hd_root -PTRIG:BYPASS=1 geant3_out_smeared.hddm -PNTHREADS=6 -PPLUGINS=ReactionFilter,photon_gun_hists,danarest,monitoring_hists -PReaction1=1_14__1_8_9_14_m0 -PReaction1:Flags=F0_B8_T9 -PReaction1:Name=gamma_pi+pi-p_nofit -PReaction2=1_14__8_9_14_m0 -PReaction2:Flags=F0_B8_T9 -PReaction2:Name=pi+pi-p_nofit -PCOMBO:MAX_NEUTRALS=20 \n")
		if(not HADRONIC_CLUSTER_VETO): myfile.write("hd_root -PFCAL:CUT_PAR1=-100 -PFCAL:CUT_PAR2=-100 -PTRIG:BYPASS=1 geant3_out_smeared.hddm -PNTHREADS=6 -PPLUGINS=ReactionFilter,photon_gun_hists,danarest,monitoring_hists -PReaction1=1_14__1_8_9_14_m0 -PReaction1:Flags=F0_B8_T9 -PReaction1:Name=gamma_pi+pi-p_nofit -PReaction2=1_14__8_9_14_m0 -PReaction2:Flags=F0_B8_T9 -PReaction2:Name=pi+pi-p_nofit -PCOMBO:MAX_NEUTRALS=20 \n")
		# myfile.write("rm hdgeant.rz geant.hbook geant3_in.hddm geant3_out.hddm smear.root \n")
		myfile.write(" 	")
		myfile.write("\n")
	
	shell_exec("chmod +x ReplaceAndRun.csh","")
	if(not gen_interactive): shell_exec("qsub -N g_pipip_gun ReplaceAndRun.csh","")
...
def PruneGenr8File(fname_in,fname_out,e_gamma_curr,theta_curr,verbose = False):
	...
	for line in open(fname_in, 'r'):	
		...
		if(counter%9==5): #Pi+ p4
			value_in_line=1
			p4_string = line.split()
			p4_pipl   = TLorentzVector(float(p4_string[1]),float(p4_string[2]),float(p4_string[3]),float(p4_string[4]))
			for value in line.split():
				if(verbose and value_in_line==1): print "Charge: "+value
				if(verbose and value_in_line==2): print "Px "+value
				if(verbose and value_in_line==3): print "Py "+value
				...
				if(verbose and value_in_line==5): print "E "+value
				value_in_line+=1
			line7_out = "   1 "+str(p4_pipl.Px()) + " " + str(p4_pipl.Py()) + " " + str(p4_pipl.Pz()) + " " + str(p4_pipl.E())+"\n"
		...
		if(counter%9==7): #Pi- p4
			value_in_line=1
			p4_string = line.split()
			p4_pim    = TLorentzVector(float(p4_string[1]),float(p4_string[2]),float(p4_string[3]),float(p4_string[4]))
			for value in line.split():
				if(verbose and value_in_line==1): print "Charge: "+value
				if(verbose and value_in_line==2): print "Px "+value
				if(verbose and value_in_line==3): print "Py "+value
				...
				if(verbose and value_in_line==5): print "E "+value
				value_in_line+=1
			line9_out = "   -1 "+str(p4_pim.Px()) + " " + str(p4_pim.Py()) + " " + str(p4_pim.Pz()) + " " + str(p4_pim.E())+"\n"
