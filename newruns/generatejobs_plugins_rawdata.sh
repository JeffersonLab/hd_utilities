...
#
# This script will run over rawdata files as they appear on
# the JLab tape system.
#
# Usage:
# generatejobs_plugins_rawdata.sh [year] [month] [day] [minrun] [maxrun] (minfile) (maxfile)
#
# User will specify the year, month, day of previous Friday run.
# This date should be consistent with what is used for setup.sh
#
...
# Run #s to iterate over for this run.
set MINRUN     = $4
set MAXRUN     = $5
...
if ($1 == "" || $2 == "" || $3 == "" || $4 == "" || $5 == "" ) then
  ...
  echo "Usage:"
  echo "generatejobs_plugins_rawdata.sh [year] [month] [day] [minrun] [maxrun] (minfile) (maxfile)"
  exit
endif
...
while (${RUN} <= ${MAXRUN})
	...
	set THIS_DIR          = /home/gxproj1/halld/monitoring/newruns/
...
#,PS_online,PSC_online"
	set SCRIPTFILE        = "/home/gxproj1/halld/monitoring/newruns/script.sh"
...
#	#---------------------------------------------------------------------------------------
	...
	while (${FILENUM} <= ${MAXFILE})
		...
		echo '			<Command>./script.sh ${env} '${INPUT_PREFIX}'${input_file_stubs}'${INPUT_SUFFIX}' ${plugins} ${bfield_option} ${additional_option} ${formatted_run} ${formatted_file} '${OUTDIR}' </Command>' >> ${OUTPUT_FILE}
