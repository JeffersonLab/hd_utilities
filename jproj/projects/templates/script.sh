...
date
set START = `date +%s`
hd_root $INPUTFILE -PPLUGINS=$PLUGINS ${ADDITIONAL_OPTION} -PNTHREADS=$NTHREADS -PTHREAD_TIMEOUT=300 -PPRINT_PLUGIN_PATHS=1 -PEVIO:ENABLE_DISENTANGLING=0 # -PEVENTS_TO_KEEP=100
echo "ending plugins ............."
date
set END = `date +%s`
set TIMEDIFF = `expr $END - $START`
