...
class EvioFileStatusType(Enum):
  """Enum for non-standard situations when getting EVIO file."""
  NO_RCDB_FILES_COUNT  = auto()
  NO_FILES_FOUND       = auto()
  MISSING_EVIO_FILE    = auto()
  UNEXPECTED_EVIO_FILE = auto()
  EXCLUDED_BY_RCDB     = auto()

@dataclass
class EvioFileStatus:
  """Class holding status information of EVIO file."""
  statusType: EvioFileStatusType
...
def get_evio_file_paths_for_run(
...
) -> tuple[list[str], list[EvioFileStatus]]:
  ...
  evio_file_statuses: list[EvioFileStatus] = []
  if evio_files_count is None:
    print(f"WARNING: RCDB does not contain a value for `evio_files_count` for run {run_number}")
    evio_file_statuses.append(EvioFileStatus(statusType = EvioFileStatusType.NO_RCDB_FILES_COUNT))
    ...
    for evio_file_index in range(evio_files_count):
      ...
      ignore_evio_file = evio_last_files_excluded is not None and evio_file_index >= evio_files_count - evio_last_files_excluded
      if ignore_evio_file:
        print(f"Excluding EVIO file '{evio_file_path}' according to RCDB field 'evio_last_files_excluded'")
        evio_file_paths_pattern.remove(evio_file_path)  # remove existing file from list generated by pattern matching
        evio_file_statuses.append(EvioFileStatus(statusType = EvioFileStatusType.EXCLUDED_BY_RCDB, filePath = evio_file_path))
      else:
        if not os.path.isfile(evio_file_path):
          print(f"WARNING: expected EVIO file '{evio_file_path}' does not exist")
          evio_file_statuses.append(EvioFileStatus(statusType = EvioFileStatusType.MISSING_EVIO_FILE, filePath = evio_file_path))
        else:
    ...
    if len(evio_file_paths_pattern) > 0:
      for evio_file_path in evio_file_paths_pattern:
        ...
        evio_file_statuses.append(EvioFileStatus(statusType = EvioFileStatusType.UNEXPECTED_EVIO_FILE, filePath = evio_file_path))
  if len(evio_file_paths) == 0:
    print(f"WARNING: could not find any EVIO files for run {run_number}")
    evio_file_statuses.append(EvioFileStatus(statusType = EvioFileStatusType.NO_FILES_FOUND))
    return [], evio_file_statuses
  return evio_file_paths, evio_file_statuses
...
def get_evio_file_paths(
  ...
  raw_data_root:                str,                # root dir for EVIO files
  missing_evio_files_list_file: str,                # path to file where the list of missing EVIO files will be written to
) -> dict[int, list[str]]:
  """Gets the list of EVIO files for each run in the given list of runs, and also tallies any non-standard file statuses encountered in the process."""
  ...
  for run_number in run_list:
    ...
    # get list of evio files for this run, and any non-standard file statuses encountered in the process
    evio_file_paths:           list[str]            = []
    evio_file_statuses_in_run: list[EvioFileStatus] = []
    evio_file_paths, evio_file_statuses_in_run = get_evio_file_paths_for_run(run_info, raw_data_root)
    evio_file_paths_per_run[run_number] = evio_file_paths
    evio_file_statuses += evio_file_statuses_in_run
  total_nmb_evio_files = sum(len(file_paths) for file_paths in evio_file_paths_per_run.values())
  # tally all non-standard file statuses
  evio_file_status_counts = Counter((err.statusType for err in evio_file_statuses))
  print(textwrap.dedent(f"""
    ...
    --------------------------------------------------------------------------------------------
      {evio_file_status_counts[EvioFileStatusType.NO_RCDB_FILES_COUNT ]:5d} runs with no `evio_files_count` value in RCDB
              for these runs the number of EVIO files was estimated by counting files in `/mss`
      {evio_file_status_counts[EvioFileStatusType.NO_FILES_FOUND      ]:5d} runs with no EVIO files found
              for these runs no jobs will be submitted
      {evio_file_status_counts[EvioFileStatusType.EXCLUDED_BY_RCDB    ]:5d} EVIO files excluded by RCDB field 'evio_last_files_excluded'
              for these files no jobs will be submitted
      {evio_file_status_counts[EvioFileStatusType.MISSING_EVIO_FILE   ]:5d} EVIO files not found
              for these files no jobs will be submitted
  ...
  if missing_evio_files:
    print(f"Writing list of missing EVIO files to '{missing_evio_files_list_file}'")
    try:
      with open(missing_evio_files_list_file, mode = "x") as file:
        for missing_file in missing_evio_files:
          file.write(f"{missing_file}\n")
    except FileExistsError:
      print(f"WARNING: file '{missing_evio_files_list_file}' already exists; skipping write")
...
def main(args: argparse.Namespace) -> None:
  ...
  evio_file_paths_per_run: dict[int, list[str]] = get_evio_file_paths(
    db                           = db,
