...
date
set START = `date +%s`
hd_root $INPUTFILE -PPLUGINS=$PLUGINS -PNTHREADS=$NTHREADS -PTHREAD_STALL_WARN_TIMEOUT=30 -PTAGMHit:PEAK_CUT=48 -PTAGMHit:USE_ADC=1 -PCOMBO:TRACK_SELECT_TAG= -PCOMBO:SHOWER_SELECT_TAG= -PCOMBO:MAX_NEUTRALS=999
set RETURN_CODE = $?
...
foreach IDXA_FILE (`ls | grep idxa`)
	...
	cp -v $IDXA_FILE ${OUTDIR}/IDXA/${RUN_NUMBER}/${BASE_NAME}_${RUN_NUMBER}_${FILE_NUMBER}.idxa
end
