...
file_name = "/diphotonCCAL.txt" # "/diphotonCCAL_single.txt"
...
#        theta1 = math.radians(random() * 0.41 + 0.14) #MCW is in radians (From vertex)
#        theta2 = math.radians(random() * 0.41 + 0.14) #MCW is in radians
        theta1 = math.radians(random() * 1.01 + 0.42) #MCW is in radians (From (0,0,800)
        theta2 = math.radians(random() * 1.01 + 0.42) #MCW is in radians
        phi = math.radians(random() * 360) # Some small range in theta and phi, so we can look at various distances between the two showers
        phi2 = math.radians(random() * 360)
        p1 = 3 #GeV
        p2 = 3 # GeV
