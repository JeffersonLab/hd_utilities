...
#CALIB_PLUGINS=HLDetectorTiming,evio_writer,pi0fcalskim,pi0bcalskim,TOF_calib,omega_skim,TAGH_timewalk,il_clb_skim
CALIB_PLUGINS=HLDetectorTiming,evio_writer,pi0fcalskim,pi0bcalskim,TOF_calib,omega_skim,TAGH_timewalk
#CALIB_OPTIONS="  -Pjana:nevents=10000  "
#CALIB_OPTIONS="  -PFCAL:LOAD_NONLIN_CCDB=0 -PFCAL:expfit_param1=2 -PFCAL:expfit_param2=0 -PFCAL:expfit_param3=0 -PPI0FCALTOFSKIM:WRITE_EVIO=1 -PPI0FCALTOFSKIM:WRITE_HDDM=0 -PPI0FCALTOFSKIM:TURN_OFF_TRACK_MATCH=1 -PPI0FCALTOFSKIM:SAVE_BEAM_PHOTON=1 -PPI0FCALTOFSKIM:SAVE_TOF=1 -PPI0FCALTOFSKIM:GET_IP=1 -PTRKFIT:FAST_TRACKING_MODE=1 "

CALIB_OPTIONS=" -PTRKFIT:FAST_TRACKING_MODE=1 -PPI0FCALSKIM:TRACKING_SELECTIONS=1 "
...
timeout 14400 hd_root -Pnthreads=$NTHREADS  -PEVIO:RUN_NUMBER=${RUNNUM} -PPRINT_PLUGIN_PATHS=1 -PTHREAD_TIMEOUT=300 -POUTPUT_FILENAME=$OUTPUT_FILENAME -PPLUGINS=$CALIB_PLUGINS $CALIB_OPTIONS $EVIOFILE
retval=$?
...
# remove job
echo ==update mysql DB== 
echo mysql -h hallddb.jlab.org -u calibInformer calibInfo -e "DELETE FROM running_jobs WHERE run=$RUN and file=$FILE"
mysql -h hallddb.jlab.org -u calibInformer calibInfo -e "DELETE FROM running_jobs WHERE run=$RUN and file=$FILE"
...
cp -v  hd_rawdata_${RUNNUM}_${FILENUM}.pi0fcalskim.evio ${SKIM_DIR}/FCAL_pi0/Run${RUN}/hd_rawdata_${RUNNUM}_${FILENUM}.pi0fcalskim.evio
chmod g+wx ${SKIM_DIR}/FCAL_pi0/Run${RUN}/
chmod g+w ${SKIM_DIR}/FCAL_pi0/Run${RUN}/hd_rawdata_${RUNNUM}_${FILENUM}.pi0fcalskim.evio
