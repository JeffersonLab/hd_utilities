...
void MonitoringToRoot_getData(TString startDate = "2014-11-14 01:00:00", TString endDate = "2014-12-21 13:00:00"){
  ...
  // Nominal time slice: 30 minute chunks
  n = n/30;
  // 1 hrour chunks
  //n = n/60;
  ...
  //TString suffix[] = {":vmon", ":temp", ":imon", ":dyn", ":mvb", ":mvt", ":dac"};
  //TString suffix[] = {":vmon", ":temp", ":imon", ":dac"};
  TString suffix[] = {":vmon", ":dyn", ":temp", ":imon", ":dac", ":status"};
  int n_suffix = sizeof(suffix)/sizeof(*suffix);
  TString Tcommand = "#!/bin/csh \n\n /usr/csite/certified/bin/mySampler -b \""+startDate+"\" -s 1h -n "+StringUtilities::int2TString(n);
  ...
  string get_values = "/gluonfs1/gluex/Subsystems/FCAL/FCAL_Analysis/MonitoringData/getBaseValues.sh";
  ...
  *outFile << Tcommand << endl;
  ...
  string outString = "/gluonfs1/gluex/Subsystems/FCAL/FCAL_Analysis/MonitoringData/getBaseValues.sh > "+StringUtilities::TString2string(ToutString);
  TStopwatch watch;
  watch.Start();
  cout << outString << endl;
