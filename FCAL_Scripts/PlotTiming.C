...
#include "TProfile.h"
#include "TSystem.h"
//root -l -q -b 'PlotTiming.C+("../RunData/hd_rawdata_002153_000.evio.root","2153")'
void PlotTiming(TString filename = "0", TString plotlabelst="default", 
...
//	uint mincrate = 35, maxcrate = 35, minslot = 9, maxslot = 11, minchan = 0, maxchan = 15;
	...
	if (filename == "0" || plotlabelst == "default") {
		printf("\nUsage:\n\n\t.x PlotTiming.C+(TString filename, TString plotlabelst, Int_t numbins = 400, Bool_t globalscale = 1, Bool_t test=0)\n\n");
		printf("\t filename \t = %s\n",filename.Data());
		printf("\t plotlabel \t = %s\n",plotlabel);
		printf("\t numbins \t = %i\n",numbins);
   ...
   // gStyle->SetOptStat(2210);
    ...
    TTree* tree = (TTree*)file->Get("Df250WindowRawData");
  ...
  tree->SetBranchAddress("waveform",&waveform);	
    tree->SetBranchAddress("nsamples",&nsamples);
      tree->SetBranchAddress("w_integral",&w_integral);
      tree->SetBranchAddress("w_min",&w_min);
      tree->SetBranchAddress("w_max",&w_max);
      tree->SetBranchAddress("w_samp1",&w_samp1);
      tree->SetBranchAddress("w_ped",&w_ped);
      tree->SetBranchAddress("w_time",&w_time);
      tree->SetBranchAddress("invalid_samples",&invalid_samples);
    ...
    for (Int_t entry=startentrynum; entry<endentrynum; entry++){
		...
		if (entry%100000==0 || endentrynum-entry<5) {
			printf("Entry %10i    %2i %2i %2i   channelnum %4i  event %6i time %2i\n",
				   entry,rocid,slot,channel,channelnum,eventnum,w_time);
		}
		...
		if (rocid >= mincrate && rocid <= maxcrate && slot >= minslot && slot <= maxslot && channel >= minchan && channel <= maxchan ) {
			if (w_time>0) {
				...
				printf("Entry %10i    %2i %2i %2i   channelnum %4i  event %6i  time %f\n",
				 	   entry,rocid,slot,channel,channelnum,eventnum,w_time);
				timehistos[rocid][slot][channel]->Fill(w_time);
			}
		}
	}
    // ************
