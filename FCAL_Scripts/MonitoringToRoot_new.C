...
void MonitoringToRoot_getData(TString startDate = "2014-11-14 01:00:00", TString endDate = "2014-12-21 13:00:00"){
  ...
  //TString suffix[] = {":vmon", ":temp", ":imon", ":v0set", ":mvb", ":mvt", ":dac"};
  //TString suffix[] = {":vmon", ":temp", ":imon", ":dac"};
  TString suffix[] = {":vmon", ":v0set", ":temp", ":imon", ":dac", ":status"};
  ...
  string get_values = "/gluonfs1/gluex/Subsystems/FCAL/FCAL_Analysis/MonitoringData/getBaseValues2.sh";
  ...
  *outFile << Tcommand << endl;
  ...
  string outString = "/gluonfs1/gluex/Subsystems/FCAL/FCAL_Analysis/MonitoringData/getBaseValues2.sh > "+StringUtilities::TString2string(ToutString);
  TStopwatch watch;
  watch.Start();
  cout << outString << endl;
...
void MonitoringToRoot(TString inputFile){
  ...
  /*
  if (s_value.EndsWith(":vmon") || s_value.EndsWith(":imon") || s_value.EndsWith(":temp")) s_value.Remove(s_value.Length()-4);
  ...
  */
    ...
    for (int i=0; i<tokens->GetEntries(); i++){
      ...
      // Year-Month-Day Hour:Minute:Second va1 val2 ...
        ...
        if (i==0) date = s_value;
          ...
          if (name.EndsWith(":temp")) {suffix = "temp"; name.Remove(name.Length()-5);}
          ...
          if ( prev_name.Contains(name+":") ){
            for (int j=int(adc_vec.size())-1; j>=0; j--){
              if (adc_vec[j].absNum==name+"_"+date){
                ...
                if (suffix == "v0set")  adc_vec[j].dynode      = s_value.Atof();
                if (suffix == "imon") adc_vec[j].current     = s_value.Atof();
                if (suffix == "temp") adc_vec[j].temperature = s_value.Atof();
            ...
            if (suffix == "v0set")  my_channel.dynode      = s_value.Atof();
            if (suffix == "imon") my_channel.current     = s_value.Atof();
            if (suffix == "temp") my_channel.temperature = s_value.Atof();
  ...
  for (int i=0; i<nADC; i++){
    ...
    if (create_ttree){
      ...
      tr->Branch("v0set",&empty_channel.dynode,"Volts/D");
    ...
    // Fill branches
    string temp = StringUtilities::TString2string(my_channel.date);
