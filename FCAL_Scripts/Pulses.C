...
*/
void Pulses_triggerStudy(TString fileName, int start_event, int custom_max = 100, bool get_waveform = true, bool sortMaxBigToSmall = true){
  ...
  for (int i=0; i<nList; i++){
    ...
    double MY_INTEGRAL = MY_CHANNEL.wIntegral; // Not Pedestal Subtracted!!
    // Subtract Pedestal
    MY_INTEGRAL += -100.0*double(MY_CHANNEL.nSamples);
    if (MY_INTEGRAL<=0) MY_INTEGRAL = 0;
    if (MY_MAX<custom_max) continue;
  ...
  for (int i=0; i<nBases; i++){
    ...
    MY_INTEGRAL += -100.0*double(MY_CHANNEL.nSamples);
    if (MY_INTEGRAL<=0) MY_INTEGRAL = 0;
    cout << "Event = " << MY_EVENT << "\t" << MY_X << "\t" << MY_Y << endl;
